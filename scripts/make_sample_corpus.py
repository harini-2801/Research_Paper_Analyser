"""
Generate a small synthetic corpus of research-paper PDFs.

The corpus exists so the pipeline can be run, demonstrated and tested without
sourcing real papers. It is written to contain the exact structure the analyser
looks for:

- standard section headings, so ingestion has something to segment on
- datasets and metrics shared across papers, which become bridge entities
- deliberately conflicting reported numbers, which contradiction detection
  should surface (papers 1 and 2 disagree on ResNet-50/CIFAR-10, papers 3 and 4
  disagree on BERT/SQuAD)
- reference lists that cite other papers in the corpus, which drives the
  citation dimension of the relationship score
- explicit gap language in papers 5 and 6
...
def main() -> int:
    ...
    written = [build_pdf(paper, out_dir) for paper in PAPERS]

    print(f"Wrote {len(written)} PDFs to {out_dir.resolve()}")
    for path in written:
        print(f"  - {path.name}")
    print("\nNext: rpra run --extraction-backend heuristic")
    return 0
